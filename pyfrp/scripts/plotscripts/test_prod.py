...
if load_emb==1:
	emb_ideal=emb_ideal.load_embryo(cwd+"/testdata/"+"testembryo_noprod.pk")
	emb=emb.load_embryo(cwd+"/testdata/"+"testembryo_prod.pk")
...
if analyze==1:
	...
	emb_ideal.save_embryo(cwd+"/testdata/"+"testembryo_noprod.pk")
	emb = analyze_dataset(emb)
	emb.save_embryo(cwd+"/testdata/"+"testembryo_prod.pk")	
	
	
	
if simulate==1:
	#Simulation	
	#emb_ideal.slice_width_px=3
	#emb_ideal.D=50/(emb.conv_fact**2)
	#emb_ideal.prod=0
	#emb_ideal=diff_reg(emb_ideal)
	#emb_ideal.save_embryo(cwd+"/testdata/"+"testembryo_ideal.pk")
	
	
	emb.slice_width_px=3
...
if fit==1:
	...
	emb_ideal.fits[0].opt_meth="TNC"
	emb_ideal.fits[0].fit_prod=0
	emb_ideal.fits[0].fit_degr=0
	emb_ideal.fits[0].fit_pinned=0
	emb_ideal.fits[0].fit_slice=0
	emb_ideal.fits[0].fit_squ=1
	emb_ideal.fits[0].fit_out=0
	emb_ideal.fits[0].UB_D=400
	emb_ideal.fits[0].x0=[20,0,0]
	emb_ideal.fits[0].equ_on=0
...
#emb.fits[0].plot_fit_unpinned()
#emb_ideal.fits[0].plot_fit_unpinned()
...
#ax.text(0.5,-0.13,ylabel,transform=ax.transAxes,ha='center',va='center') 

plt.draw()
plt.savefig('/home/alex_loc/Documents/Talks/zebrafish2014/figs/bar_react.eps')
#plt.show()
